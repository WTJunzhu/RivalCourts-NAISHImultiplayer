...
function requireTurn(state:GameState,playerId:string){ if(state.phase!=='PLAYING') throw new Error('现在不能行动'); if(currentPlayer(state).id!==playerId) throw new Error('还没轮到你'); }
...
export function applyAction(state:GameState,playerId:string,action:ActionPayload){ requireTurn(state,playerId); const player=currentPlayer(state); const limits=SLOT_LIMITS[state.gridSize];
  if(action.type==='RECRUIT'){
    normalizeCol(action.talentCol,state.gridSize); normalizeCol(action.targetCol,state.gridSize); if(isLockedCenter(player,action.targetZone,action.targetCol,state.gridSize)) throw new Error('宣国后的君主位已锁定'); const stack=state.talent[action.talentCol-1]; const card=stack.cards.shift(); if(!card) throw new Error('该英才叠已耗尽'); const old=cardAt(player,action.targetZone,action.targetCol); setCard(player,action.targetZone,action.targetCol,card); log(state,`${player.name} 招贤 ${card.type}，替换 ${old.type}`);
    if(action.swap){ if(state.slots.swap>=limits.swap) throw new Error('交换槽已满'); spendEnvoy(player); state.slots.swap++; player.slotsUsed.swap++; const a=action.swap,b=action.swap; normalizeCol(a.colA,state.gridSize); normalizeCol(b.colB,state.gridSize); if(isLockedCenter(player,a.zoneA,a.colA,state.gridSize)||isLockedCenter(player,b.zoneB,b.colB,state.gridSize)) throw new Error('不能移动锁定君主位'); const ca=cardAt(player,a.zoneA,a.colA); setCard(player,a.zoneA,a.colA,cardAt(player,b.zoneB,b.colB)); setCard(player,b.zoneB,b.colB,ca); log(state,`${player.name} 附加交换`); }
    if(action.discard){ if(state.slots.discard>=limits.discard) throw new Error('弃牌槽已满'); spendEnvoy(player); state.slots.discard++; player.slotsUsed.discard++; for(const col of [action.discard.talentColA,action.discard.talentColB]){ normalizeCol(col,state.gridSize); state.talent[col-1].cards.shift(); } log(state,`${player.name} 附加弃牌`); }
