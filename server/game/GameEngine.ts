...
export function applyAction(state:GameState,playerId:string,action:ActionPayload){ requireTurn(state,playerId); const player=currentPlayer(state); const limits=SLOT_LIMITS[state.gridSize];
  if(action.type==='RECRUIT'){
    ...
    if(peekNum%2===0){ const revealCount=peekNum/2; state.pendingPrompt={type:'PEEK_REVEAL',targetPlayerId:action.targetPlayerId,revealCount}; log(state,`${player.name} 已第 ${peekNum} 次偷看 ${peekTarget.name}，需主动展示 ${revealCount} 张暗室牌`); }
