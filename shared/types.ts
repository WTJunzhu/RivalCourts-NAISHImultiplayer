...
export type RecruitAction={type:'RECRUIT'; talentCol:number; targetZone:Zone; targetCol:number; swap?:{zoneA:Zone; colA:number; zoneB:Zone; colB:number}; discard?:{talentColA:number; talentColB:number}};
export type ActionPayload=RecruitAction|{type:'ENVOY'; targetPlayerId:string}|{type:'COMMAND'; myZone:Zone; myCol:number; targetPlayerId:string}|{type:'RECALL'}|{type:'DECLARE_NATION'; nation:Nation}|{type:'HEGEMONY'}|{type:'HISTORY_CARD'; cardType:HistoryCardType; params:Record<string,unknown>};
